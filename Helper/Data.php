...
{% endfor %}';
    ...
    public function getProductsData($feed)
    {
	    ...
	    $defaultUrl = $this->storeManager->getStore()->getBaseUrl();
	    $currentUrl = $this->storeManager->getStore($feed->getStoreId())->getBaseUrl();
	    if (strpos($productLink, $defaultUrl) >= 0 ) {    $productLink = str_replace($defaultUrl, $currentUrl, $productLink);}
		...
		$imageUrl = '';
	    	if (strpos($image->getUrl(), $defaultUrl) >=0 ){
			$imageUrl = str_replace($defaultUrl, $currentUrl, $image->getUrl());
		}
